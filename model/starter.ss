...
#C setup of welsh crab stock
data.ss
control.ss
0 # 0=use init values in control file; 1=use ss.par
2 # run display detail (0 = minimal; 1=one line per iter; 2=each logL)
...
-1 # min yr for sdreport outputs (-1 for styr); #_1960
-2 # max yr for sdreport outputs (-1 for endyr+1; -2 for endyr+Nforecastyrs); #_2034
...
2 # Depletion basis:  denom is: 0=skip; 1=X*SPBvirgin; 2=X*SPBmsy; 3=X*SPB_styr; 4=X*SPB_endyr; 5=X*dyn_Bzero;  values>=11 invoke N multiyr (up to 9!) with 10's digit; >100 invokes log(ratio)
...
1234 # random number seed for bootstrap data (-1 to use long(time) as seed): # 1234
