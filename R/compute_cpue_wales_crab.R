# script for computing nominal total catch, effort, landings, CPUE, and LPUE for the observer dataset - wales-crab
...
# create lobster as covariates
observer_data_lobster_cov <- observer_data_lobster_out[[2]] |>
  dplyr::rename(nominal.effort_potset_lobster = nominal.effort_potset,
                nominal.catch_potset_lobster = nominal.catch_potset,
                nominal.landing_potset_lobster = nominal.landing_potset,
                nominal.cpue_potset_lobster = nominal.cpue_potset,
                nominal.lpue_potset_lobster = nominal.lpue_potset,
                nominal.discard_potset_lobster = nominal.discard_potset, 
                nominal.berried_potset_lobster = nominal.berried_potset,
                nominal.recruit_potset_lobster = nominal.recruit_potset,
                nominal.berried.cpue_potset_lobster = nominal.berried.cpue_potset
                ) |>
  dplyr::select(-species) |>
  dplyr::glimpse()
# quick tally on effort
observer_data_lobster_cov |> dplyr::group_by(fisher) |>
  dplyr::reframe(min_pot = min(nominal.effort_potset_lobster, na.rm = TRUE),
                 mean_pot = mean(nominal.effort_potset_lobster, na.rm = TRUE),
                 max_pot = max(nominal.effort_potset_lobster, na.rm = TRUE))

# merge with the main datasets
observer_data_crab_out[[2]] <- observer_data_crab_out[[2]] |>
  dplyr::full_join(observer_data_lobster_cov) |>
  dplyr::mutate(nominal.effort_potset = dplyr::case_when(is.na(nominal.effort_potset) & !is.na(nominal.effort_potset_lobster) ~ nominal.effort_potset_lobster,
                                                       !is.na(nominal.effort_potset) & !is.na(nominal.effort_potset_lobster) ~ nominal.effort_potset, 
                                                       !is.na(nominal.effort_potset) & is.na(nominal.effort_potset_lobster) ~ nominal.effort_potset),
                nominal.cpue_potset = dplyr::case_when(is.na(nominal.cpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ 0,
                                                       !is.na(nominal.cpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ nominal.cpue_potset, 
                                                       !is.na(nominal.cpue_potset) & is.na(nominal.cpue_potset_lobster) ~ nominal.cpue_potset),
                nominal.recruit_potset = dplyr::case_when(is.na(nominal.recruit_potset) & !is.na(nominal.cpue_potset_lobster) ~ 0,
                                                          !is.na(nominal.recruit_potset) & !is.na(nominal.cpue_potset_lobster) ~ nominal.recruit_potset,
                                                          !is.na(nominal.recruit_potset) & is.na(nominal.cpue_potset_lobster) ~ nominal.recruit_potset),
                nominal.lpue_potset = dplyr::case_when(is.na(nominal.lpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ 0,
                                                       !is.na(nominal.lpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ nominal.lpue_potset,
                                                       !is.na(nominal.lpue_potset) & is.na(nominal.cpue_potset_lobster) ~ nominal.lpue_potset),
                nominal.berried.cpue_potset = dplyr::case_when(is.na(nominal.berried.cpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ 0,
                                                             !is.na(nominal.berried.cpue_potset) & !is.na(nominal.cpue_potset_lobster) ~ nominal.berried.cpue_potset,
                                                             !is.na(nominal.berried.cpue_potset) & is.na(nominal.cpue_potset_lobster) ~ nominal.berried.cpue_potset)) |>
  dplyr::mutate(nominal.cpue_potset_lobster = dplyr::case_when(is.na(nominal.cpue_potset_lobster) ~ 0,
                                                            !is.na(nominal.cpue_potset_lobster) ~ nominal.cpue_potset_lobster),
                nominal.catch_potset_crab = dplyr::case_when(is.na(nominal.catch_potset_lobster) ~ 0,
                                                             !is.na(nominal.catch_potset_lobster) ~ nominal.catch_potset_lobster),
                nominal.landing_potset_crab = dplyr::case_when(is.na(nominal.landing_potset_lobster) ~ 0,
                                                               !is.na(nominal.landing_potset_lobster) ~ nominal.landing_potset_lobster),
                nominal.lpue_potset_crab = dplyr::case_when(is.na(nominal.lpue_potset_lobster) ~ 0,
                                                            !is.na(nominal.lpue_potset_lobster) ~ nominal.lpue_potset_lobster)) |>
  dplyr::glimpse()

# export output as rds (as a list)
readr::write_rds(observer_data_crab_out, file = "observer_data_crab_nominal.cpue.rds") 
# export output as csv
readr::write_csv(observer_data_crab_out[[1]], file = "observer_data_crab_nominal.cpue_trip.csv")
readr::write_csv(observer_data_crab_out[[2]], file = "observer_data_crab_nominal.cpue_potset.csv")


# plot output
# temporal variation
# select a dataset and a parameter
data <- observer_data_crab_out[[2]] |> 
...
observer_data_crab_yr |> ggplot2::ggplot(ggplot2::aes(x=as.numeric(year), y=obj, group = escape_gaps, color = escape_gaps)) +  
...
# export plot
ggplot2::ggsave(file=paste0("plots/wales/", "observer_trends_wales_", response.name, "_", unique(data$species), ".svg"), 
...
shp_ices.subrec <- sf::read_sf(dsn = "ICES_Sub_Rect/ICES_SubStatrec_20150113_3857.shp", stringsAsFactors = FALSE)
