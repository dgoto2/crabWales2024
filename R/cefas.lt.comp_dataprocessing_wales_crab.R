# script for cleaning the length composition dataset for the Welsh stock of brown crab  (Cefas port sampling)
...
# crab
# offshore:  "31E5" "31E4" "33E4""34E4"
# inshore: "32E4" "33E5" "35E5" "32E5" "34E5" "32E6" "35E6"

# read in data
port.sampling_crab <- readr::read_csv("port.sampling_crab_wales_ices_rec_landings.csv", 
...
# export datasets
readr::write_csv(size.data_crab_wales, file = "cefas_port-sampling_size.data_crab_wales_clean.csv") 

# minimum landings size
year <- c(1983:2024)
mls <- c(rep(115, 6), rep(130, 26), rep(140, 10))
mls <- data.frame(year, mls)
mls <- mls |> 
  dplyr::filter(!(year %in% c(1986, 1987, 1993, 1996, 2010:2015, 2021:2024)))

# plotting
# select a dataset
data_no.na <- size.data_crab_wales |> 
...
size.dist_crab_yr <- size.dist_crab |> 
  tidyr::gather(sizebin, value, f1:colnames(size.dist_crab)[ncol(size.dist_crab)], factor_key = TRUE) |>
...
# export dataset
readr::write_csv(size.dist_crab, file = "size.comp.data_crab_wales_ss_5mm.csv") 
readr::write_csv(size.dist_crab_yr, file = "size.comp.data_crab_wales_yr_ss_5mm.csv") 
