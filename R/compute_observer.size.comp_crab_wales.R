# script to reformat the observer size composition data as SS model input for the Welsh stock of brown crab
...
observer_data_crab |>
  ...
  dplyr::glimpse()
observer_cpue_crab <- readr::read_csv("observer_data_crab_nominal.cpue_potset.csv", 
                                         col_types = readr::cols(ices_sub_rect = readr::col_character())) |> 
  tidyr::unite(month.yr.rect, c(year, month, ices_sub_rect), sep = "-", remove = FALSE) |> 
...
observer_crab_discard.ratio <- observer_cpue_crab |>
  dplyr::group_by(year) |>
...
observer_cpue_crab_rect <- observer_cpue_crab |>
...
observer_data_crab <- observer_data_crab |> 
  dplyr::left_join(observer_cpue_crab_rect) |>
  dplyr::glimpse()

# split datasets into retain and discard
observer_data_crab_retain <- observer_data_crab |>
  dplyr::filter(landed == 1) |>
  dplyr::glimpse()
observer_data_crab_discard <- observer_data_crab |>
...
(plot3 <- ggplot2::ggplot(observer_data_crab_no.na, ggplot2::aes(x = carapace_length_width, y = as.factor(year))) +
    ggridges::geom_density_ridges(scale = 2.5, alpha = 0.3, quantile_lines = TRUE, quantiles = c(0.05, 0.5, 0.95), 
                                  ggplot2::aes(fill = as.factor(sex))) +
    ggplot2::geom_vline(xintercept = 140, color = "darkgreen", linewidth = 1) +
    ggplot2::xlim(50, 250) +
...
for (part in c(0, 1, 2)) {
  ...
  if (part==1) {
    data <- observer_data_crab_discard
    part_label = "discard"
  }
  if (part==2) {
    data <- observer_data_crab_retain
    part_label = "retain"
  }
  print(data)
  cat(part_label)
  ...
  (size.max <- round(max(observer_data_crab$carapace_length_width, na.rm = TRUE)/10)) # -> based on the complete data
  width <- 0.5 # bind width = 5mm
  n.size <- length(table(cut(data$length, breaks = c(size.min, seq(size.min+width, size.max-width, by = width), size.max))))
  ...
  for (i in c(unique(data$month.yr.rect))) {
    ...
    if (nrow(subdata_m) > 0) {
      ...
      size.dist_m[7:(n.size+6)] <- table(cut(subdata_m$length, breaks = c(size.min, seq(size.min+width, size.max-width, by = width), size.max))) * 
    ...
    if (nrow(subdata_f) > 0) {
      ...
      size.dist_f[7:(n.size+6)] <- table(cut(subdata_f$length, breaks = c(size.min, seq(size.min+width, size.max-width, by = width), size.max))) * 
  ...
  size.dist_crab_month <- size.dist_crab2 |>
  ...
  #print(size.dist_crab_yr)
  # export output
  readr::write_csv(size.dist_crab, file = paste0("observer.size.comp.data_crab_wales_month_ss_", part_label,".csv")) 
  readr::write_csv(size.dist_crab_yr, file = paste0("observer.size.comp.data_crab_all_yr_wales_ss_", part_label ,".csv")) 
}
